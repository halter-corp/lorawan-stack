...
func (p *DeviceManagementPackage) HandleUp(ctx context.Context, assoc *ttnpb.ApplicationPackageAssociation, up *ttnpb.ApplicationUp) error {
	...
	logger.WithField("uplink", up).Debug("Uplink sent to the Device Management Service")
	...
	err = p.server.DownlinkQueueReplace(ctx, assoc.EndDeviceIdentifiers, []*ttnpb.ApplicationDownlink{down})
	if err != nil {
		return err
	}
	logger.WithField("downlink", down).Debug("Device Management Service downlink scheduled")
